...
.shell s2/interactive-mode1.txt
...
.shell s2/interactive-mode2.txt
...
.shell s2/interactive-mode3.txt
...
.shell s2/interactive-mode4.txt
...
Git provides it's own special protocol, which is basically just a really thin wrapper over the 'git-upload-pack' command that will tell you what is available, then you tell it what you have and it gives you a packfile of the difference.  To start it up, run something like the following command:

.shell $> git daemon --detach

The git protocol has no built in authentication, so generally you cannot push over it (although some people have open push policies and so allow that - I would not recommend setting that up, so you'll have to look up how to do that).  If you want your users to have push access, it's recommended to use the ssh protocol.  Many repositories, like GitHub, have ssh enabled for account owners to push over, and git enabled for the public to pull over - which is often the most efficient combination.
...
It is generally a good idea to put this in the 
